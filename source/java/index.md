...
54. 关于常用的几个容器对象：
    ...
    HashMap：底层是数组+单向链表/红黑树，默认初始容量为0，添加第一组元素后变为16，默认装填因子为0.75，元素数量>=容量*装填因子之后开始扩容，每次扩容为之前的2倍。当元素数量>=64且桶大小>=7时，桶由链表转换为红黑树，桶大小<=6，红黑树转换为链表。key和value都可以为null，线程不安全；
